...
class STVQAAcc_metric(datasets.Metric):
    ...
    def _compute(self, predictions, references):
        ...
        for qid, answer in references.items():
            try:
                pred_answer = self.answer_processor(predictions[str(qid)]['answer'])
            except:
                pred_answer = self.answer_processor(predictions[qid]['answer'])
            gts = [self.answer_processor(a) for a in answer['answer']]
    ...
    def _compute_single(self, prediction, references):
        pred_answer = self.answer_processor(prediction)
        gts = [self.answer_processor(a) for a in references]
        score = (1 if pred_answer in gts else 0)
        return score
    ...
    def _compute_single_list(self, predictions, references):
        ...
        for qid, answer in references.items():
            try:
                pred_answers = [self.answer_processor(ans) for ans in predictions[str(qid)]['answer']]
            except:
                pred_answers = [self.answer_processor(ans) for ans in predictions[qid]['answer']]
            gts = [self.answer_processor(a) for a in answer['answer']]
