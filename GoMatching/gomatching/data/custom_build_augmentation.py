...
from .transforms.custom_augmentation_impl import EfficientDetResizeCrop, EfficientDetResizeCrop2
...
def build_custom_augmentation(cfg, is_train):
    ...
    elif cfg.INPUT.CUSTOM_AUG == 'EfficientDetResizeCrop2':
        if is_train:
            scale = cfg.INPUT.SCALE_RANGE
            size = cfg.INPUT.TRAIN_SIZE
            h = cfg.INPUT.TRAIN_H
            ...
            h = cfg.INPUT.TEST_H
            w = cfg.INPUT.TEST_W
        augmentation = [EfficientDetResizeCrop2(size, scale, h=h, w=w)]
    else:
