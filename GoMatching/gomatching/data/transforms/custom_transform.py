...
__all__ = [
    "EfficientDetResizeCropTransform", "EfficientDetResizeCropTransform2",
]
...
class EfficientDetResizeCropTransform2(Transform):
    """
    """
    ...
    def apply_image(self, img, interp=None):
        ...
        if img.dtype == np.uint8:
            ...
            right = min(self.scaled_w, self.offset_x + self.target_size[1])
            lower = min(self.scaled_h, self.offset_y + self.target_size[0])
            if (right - self.offset_x) % 32 != 0:
                diff = (right - self.offset_x) % 32
                self.offset_x += diff
            if (lower - self.offset_y) % 32 != 0:
                diff = (lower - self.offset_y) % 32
                self.offset_y += diff
            if len(ret.shape) <= 3:
                ret = ret[self.offset_y: lower, self.offset_x: right]
            else:
