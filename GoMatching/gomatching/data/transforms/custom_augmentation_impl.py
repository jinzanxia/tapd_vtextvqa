...
from .custom_transform import EfficientDetResizeCropTransform, EfficientDetResizeCropTransform2

__all__ = [
    "EfficientDetResizeCrop", "EfficientDetResizeCrop2"
]
...
class EfficientDetResizeCrop2(Augmentation):
    """
    Scale the shorter edge to the given size, with a limit of `max_size` on the longer edge.
    If `max_size` is reached, then downscale so that the longer edge does not exceed max_size.
    """
    ...
    def get_transform(self, img):
        ...
        offset_y = int(max(0.0, float(offset_y)) * np.random.uniform(0, 1))
        offset_x = int(max(0.0, float(offset_x)) * np.random.uniform(0, 1))
        return EfficientDetResizeCropTransform2(
            scaled_h, scaled_w, offset_y, offset_x, img_scale, self.target_size, self.interp)
