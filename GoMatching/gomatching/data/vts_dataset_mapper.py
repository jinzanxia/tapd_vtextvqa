...
def custom_annotations_to_instances(annos, image_size, mask_format="polygon", \
    ...
    # texts = [obj["texts"] for obj in annos]
    # texts = torch.tensor(texts, dtype=torch.int32)
    # target.texts = texts

    beziers = [obj.get("beziers", []) for obj in annos]
    target.beziers = torch.as_tensor(beziers, dtype=torch.float32)

    polys = [obj.get("polyline", []) for obj in annos]
    target.polyline = torch.as_tensor(polys, dtype=torch.float32)

    boundary = [obj.get("boundary", []) for obj in annos]
    target.boundary = torch.as_tensor(boundary, dtype=torch.float32)
...
class GoMDatasetMapper(DatasetMapper):
    @configurable
    def __init__(
        self,
        is_train: bool,
